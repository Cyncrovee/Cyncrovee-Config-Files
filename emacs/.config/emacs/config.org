...
#+BEGIN_SRC elisp
  ...
  (define-key evil-normal-state-map (kbd "C-c h") 'next-buffer)
  (define-key evil-normal-state-map (kbd "C-c l") 'previous-buffer)
  (keymap-global-set "<space>" 'execute-extended-command) ; Switch to the minibuffer
